 ...
 * @return {String}
 */
bigine.util.resource.format = function(url) {
    if (!bigine.util.isString(url)) return url;
    return bigine.runtime.env.protocol + url.replace(/^.+\/\//, '//').replace(/\?.*$/, '') + '?' + bigine.$version;
};
...
bigine.util.resource.getSuite = function(uuid) {
    return new bigine.util.promise(function (resolve, reject) {
        $.$xhr('POST', $.format('//api.dahao.de/resource/' + uuid + '/'), resolve, reject);
    });
};
...
bigine.util.resource.getTheme = function(uuid) {
    return new bigine.util.promise(function (resolve, reject) {
        $.$xhr('GET', $.format('//s.dahao.de/theme/' + uuid + '/theme.json'), resolve, reject);
    });
};
