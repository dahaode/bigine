...
bigine.util.promise.tick = function(times) {
    return $.delay(Math.round(50 * (parseInt(times, 10) || 1) / 3));
};
...
bigine.util.promise.animate = function(animate, times) {
    ...
    var counter = 1,
        ...
        classic = bigine.runtime.env.browser.msie && 10 > parseFloat(bigine.runtime.env.browser.version),
        step = classic ? 2 : 1;
    if (1 > times) {
        return promise;
    }
    for (; counter <= times; counter++) {
        if (classic && 1 + counter != times && 0 === counter % 2) continue;
        if (1 + counter == times) step = 1;
        promise = promise.tick(step).then(next(counter));
    }
    return promise;
};
