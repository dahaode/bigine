...
var bigine = {
    core: {
        tag: {
            map: require('lib/core/tag/map')
        }
    },
    error: require('lib/error'),
    util: {
        import: require('lib/util/import')
    }
},
    $ = {};
...
$.tokenize = function(lines) {
    ...
    for (ii = 0; ii < lines.length; ii++) {
        ...
        kk = {
            line: 1 + ii,
            depth: jj[1].length,
            id: bigine.core.tag.map[jj[2]] || 'bigine.core.tag',
            name: jj[2],
            params: [],
            content: jj[3] || jj[5]
        };
...
$.instantiate = function(token) {
    var tag = new (bigine.util.import(token.id))(token.line, token.params, token.content);
