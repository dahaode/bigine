...
bigine.core.episode = bigine.$extends(bigine.core.component, function (tags, resource) {
    ...
    var end = new bigine.tag.scene(0, [], '--done-end');
    end.type = new bigine.tag.scene.type(0, [], '完结时');
    end.plot = new bigine.tag.scene.plot();
    end.plot.$append(new bigine.action.episode.end());
    this.$scenes = {};
    this.$scenes[bigine.core.emittable.TYPE_EPISODE_DONE] = [end];
...
bigine.core.episode.prototype.done = function(context) {
    return this.$fire(bigine.core.emittable.TYPE_EPISODE_DONE, context).then(function () {
