...
bigine.core.runtime.prototype.$fire = function(id) {
    ...
    for (var ii = 0, jj; ii < $.on[id].length; ii++) {
        jj = ii ?
            jj.then($.on[id][ii].perform) :
            $.on[id][0].perform(context);
    }
    jj = jj.fail(function (error) {
        console.error(error);
    });
    return jj;
};
 ...
 * @return {bigine.core.runtime}
 */
bigine.core.runtime.prototype.run = function(state) {
    $.state = state;
    return this.start();
};
