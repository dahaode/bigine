 ...
 *
 * @param  {String} id
 * @return {Function(new:bigine.core.tag, ?Number, ?Array<String>, ?String)}
 * @static
 */
bigine.core.tag.factory = function(id) {
    if (!$.tags) {
        $.tags = [];
    ...
    id = bigine.core.tag.factory.$map[id] || id;
    if (-1 == $.tags.indexOf(id)) {
        return bigine.core.tag;
    }
    return require(id.replace(/\./g, '/'));
};
