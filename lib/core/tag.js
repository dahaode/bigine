...
bigine.core.tag = bigine.$extends(bigine.core.component, function (line, params, content) {
    ...
    if (expect instanceof Array && expect[1]) {
        if (this.$params.length < expect[0]) {
            throw new bigine.error('参数个数不足 ' + expect[0] + ' 个', this);
        }
        if (this.$params.length > expect[1]) {
            throw new bigine.error('参数个数超过 ' + expect[1] + ' 个', this);
        }
    } else if (this.$params.length && !expect) {
        throw new bigine.error('参数应为空', this);
    }
    this.$content = content;
    expect = this.$expectContent();
    if (content) {
        if (!expect) {
            throw new bigine.error('内容应为空', this);
        }
    } else if (line && expect && 'bigine.core.tag' != this.$prototype) {
        throw new bigine.error('内容不能为空', this);
    }
    this.$children = [];
});
...
bigine.core.tag.prototype.$register = function(episode) {
    ...
    if (!this.$children.length && expect) {
        return bigine.util.promise.reject(new bigine.error('子标签缺失', this));
    }
    if (this.$_$register) {
        return bigine.util.promise.resolve(episode);
    }
    this.$_$register = true;
    return bigine.util.promise.every(this.$children, '$register', episode);
};
...
bigine.core.tag.prototype.$bind = function(episode) {
    ...
    this.$_$bind = true;
    return bigine.util.promise.every(this.$children, '$bind', episode);
};
 ...
 * @override
 */
bigine.core.tag.prototype.toScript = function(depth) {
    depth = depth || 0;
    ...
    for (ii = 0; ii < this.$children.length; ii++) {
        clob += this.$children[ii].toScript(1 + depth);
    }
    return clob;
};
 ...
 * @return {String}
 */
bigine.core.tag.prototype.toJsrn = function(lineNumber) {
    var clob = [],
        ii = bigine.episode.index.indexOf(this.$prototype),
        jj = [];
    if (-1 == ii) {
        throw new bigine.error('未知标签“' + this.$prototype + '”', this);
    }
    clob.push(ii);
    if (this.$content) {
