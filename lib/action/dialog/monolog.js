...
bigine.action.dialog.monolog.prototype.act = function(context) {
    context.logger.info(' [episode]', this.player.title + '：『' + this.words + '』');
    return context.director.monolog(this, context);
};

/** @inheritDoc */
bigine.action.dialog.monolog.prototype.$prefetch = function(resource) {
    if (this.player.avatar) resource.register(this.player.avatar.src, 'avatar');
};
