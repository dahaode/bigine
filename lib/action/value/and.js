...
bigine.action.value.and.prototype.act = function(context) {
    ...
    context.state[$v] = this.test(context.state, context.logger) ? '真' : '伪';
    context.logger.debug('   [state] ' + $v + ' =', context.state[$v]);
    return context.director.oops(context);
};

/** @inheritDoc */
bigine.action.value.and.prototype.$prefetch = function(resource) {
