...
bigine.action.room.bgm.prototype.act = function(context) {
    ...
    context.state['当前音乐'] = this.bgm.id;
    context.logger.debug('   [state] 当前音乐 =', this.bgm.id);
    return context.director.playBGM(this, context);
};

/** @inheritDoc */
bigine.action.room.bgm.prototype.$prefetch = function(resource) {
    resource.register(this.bgm.audio.src, 'bgm');
};
