...
bigine.tag.scene.prototype.$bind = function(episode) {
    if (!this.type) {
        return bigine.util.promise.reject(new bigine.error('事件类型未定义', this.$line));
    }
    if (!this.plot) {
        return bigine.util.promise.reject(new bigine.error('事件内容不能为空', this.$line));
    }
    var self = this;
...
bigine.tag.scene.prototype.perform = function(context) {
    switch (true) {
        case this.type.id != context.state['$e']:
        case this.assertion && !this.assertion.test(context.state):
        case bigine.core.emittable.TYPE_ROOM_BEFORE_ENTER == this.type.kind && this.type.reference != context.state['$n']:
        case bigine.core.emittable.TYPE_ROOM_AFTER_ENTER == this.type.kind && this.type.reference != context.state['$r']:
        case bigine.core.emittable.TYPE_ROOM_BEFORE_LEAVE == this.type.kind && this.type.reference != context.state['$r']:
        case bigine.core.emittable.TYPE_ROOM_AFTER_LEAVE == this.type.kind && this.type.reference != context.state['$r']:
            context.logger.info('   [scene] skip', this);
            return context.director.oops(context);
    }
    context.logger.info('   [scene] perform', this);
    context.state['$s'] = this;
    context.logger.debug('   [state] $s =', this);
