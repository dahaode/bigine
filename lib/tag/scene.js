...
/** @inheritDoc */
bigine.tag.scene.prototype.toJsrn = function(lineNumber) {
    var clob = bigine.core.tag.prototype.toJsrn.call(this, lineNumber);
    if (this.id) {
        clob = clob.substr(0, clob.length - 1) + ',"' + this.id.replace(/-/, '') + '")';
    }
    return clob;
};
...
bigine.tag.scene.prototype.perform = function(context) {
    ...
    var actions = [],
        dig = function(tag) {
            if (tag.id) actions.push(tag.id);
            bigine.util.each(tag.$children, dig);
        };
    if (this.plot) dig(this.plot);
    context.dispatch(new bigine.runtime.event.scene({
        target: this,
        id: this.id,
        title: this.title,
        actions: actions
    ...
    return promise.then(function () {
        context.state['$d'] = 0;
        context.logger.debug('   [state] $d = 0');
