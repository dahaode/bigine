...
/** @inheritDoc */
bigine.tag.action.prototype.toJsrn = function(lineNumber) {
...
bigine.tag.action.prototype.$splitContent = function(max) {
    ...
    if (max && params.length > max) {
        throw new bigine.error('内容扩展参数个数超过 ' + max + ' 个', this);
    }
    return params;
};
...
bigine.tag.action.prototype.test = function(state, logger) {
    throw new bigine.error('动作“' + this.$prototype + '”检查功能逻辑未实现', this);
};
 ...
 * @return {void}
 */
bigine.tag.action.prototype.$prefetch = function(resource) {};
