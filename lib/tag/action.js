...
bigine.tag.action.prototype.test = function(state) {
    return bigine.util.promise.reject(new bigine.error('动作“' + this.$prototype + '”检查功能逻辑未实现', this.$line));
};
...
bigine.tag.action.prototype.act = function(context) {
    context.logger.error('bigine.tag.action#act(context)');
    return bigine.util.promise.reject(new bigine.error('动作“' + this.$prototype + '”逻辑未实现', this.$line));
};
