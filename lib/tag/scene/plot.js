...
bigine.tag.scene.plot.prototype.play = function(context) {
    context.state['$d']++;
    context.logger.debug('   [state] $d =', context.state['$d']);
    this.$prefetch(context.resource);
    return bigine.util.promise.every(this.$children, 'act', context, function (action) {
        context.logger.debug('  [action]', action);
    }).then(function () {
        context.state['$d']--;
        context.logger.debug('   [stated] $d =', context.state['$d']);
        return context;
    });
};
 ...
 * @return {void}
 */
bigine.tag.scene.plot.prototype.$prefetch = function(resource) {
    bigine.util.each(this.$children, function (action) {
        action.$prefetch(resource);
    });
};
