...
bigine.entity.room = bigine.$extends(bigine.tag.entity, function (line, params, content) {
    ...
    this.$scenes.fallback.type.id = bigine.core.emittable.TYPE_ROOM_AFTER_ENTER;
    this.$scenes.fallback.type.kind = bigine.core.scene.KIND_ROOM;
    this.$scenes.fallback.type.reference = this;
    this.$scenes.fallback.plot = new bigine.tag.scene.plot();
    this.$scenes.fallback.plot.$append(new bigine.action.room.fallback());
...
bigine.entity.room.prototype.$fire = function(type, context) {
    ...
    return bigine.util.promise.every(this.$scenes[type] || fallback, 'perform', context);
};
