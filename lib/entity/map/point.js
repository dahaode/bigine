...
bigine.entity.map.point.prototype.$register = function(episode) {
    try {
        ...
        if (this.target) {
            if (this.template) {
                if (!this.mapt) {
                    throw new bigine.error('地图模板缺失', this);
                }
                if (this.image || this.region) {
                    throw new bigine.error('无法重载地图模板交互点', this);
                }
                if (!this.mapt.points[this.template]) {
                    throw new bigine.error('地图模板交互点“' + this.template + '”未定义', this);
                }
        ...
        if (fault) {
            throw new bigine.error('交互点“' + this.title + '”信息不完整', this);
        }
    } catch (error) {
