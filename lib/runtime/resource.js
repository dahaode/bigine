...
bigine.runtime.resource = function(screen,audioName,srategy){
    ...
    * 预下载队列
    */
    var _queue = new bigine.runtime.resource.queue();
     ...
     *
     */
    var loadSource = function(uuid,type,source){
        if(fetchedList[uuid]){
            _queue.del(uuid);
            console.log("loadSource==="+uuid);
    ...
    var fetchSource = function(index,num){
        ...
        // if("undefined" == typeof num){
            for(var i = 0 ; i<index;i++ ){
                ...
                if(!source.status){
                    loadSource(source.uuid,source.type,source);
                }
    ...
    var performanceSrategy = function(){
        setInterval(function(){
            var prefetchQ = prefetchQ.getPrefetchQ();
    ...
    var balanceSrategy = function(){
        setInterval(function(){
            ...
            // var prefetchQ = _queue.getPrefetchQ()
            // for(var i = 0 ; i < (_queue.length()<30?_queue.length():30) ; i++){
            //     var source = prefetchQ[i];
            //     //console.log(source.uuid+"  "+source.status);
 ...
 * 资源预加载队列。
 */
bigine.runtime.resource.queue = function(){
    /**
     * 资源下载队列，只存放未下载资源
     *
     * @type array
