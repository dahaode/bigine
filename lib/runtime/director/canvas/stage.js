...
bigine.runtime.director.canvas.stage = bigine.$extends(bigine.core.component, function (runtime, canvasContext) {
    ...
    bigine.util.on(this.canvas, {
        mousemove: function(event) {
            ...
            if (self.$curtain || !self.$map) return false;
...
bigine.runtime.director.canvas.stage.prototype.render = function() {
    var self = this,
        w = 1280,
        h = 720;
    if (this.$cache) this.$ctx.putImageData(this.$cache, 0, 0);
    else {
        ...
        this.$cache = this.$ctx.getImageData(0, 0, w, h);
    }
    if (this.$map) {
        if (this.$start) {
...
bigine.runtime.director.canvas.stage.prototype.setLayer = function(index, image) {
    ...
    if (5 > index) this.$cache = undefined;
    return this;
};
...
bigine.runtime.director.canvas.stage.prototype.animateLayer = function(index, image) {
    ...
    return promise.animate(function (elapsed, duration) {
        ...
        if (1 < elapsed) self.$ctx.putImageData(state, 0, 0);
...
bigine.runtime.director.canvas.stage.prototype.setChoose = function(points, theme) {
    ...
    bigine.util.each(points, function (point, index) {
        self.$font(32, theme.color, theme.shadow, function () {
            ...
            q.push(new promise(function (resolve) {
                bigine.util.on(points[index].i, 'load', function () {
                    resolve();
                });
            }));
            ...
            ctx.fillText(point.t, x, y);
            q.push(new promise(function (resolve) {
                bigine.util.on(points[index].ih, 'load', function () {
                    resolve();
                });
            }));
...
bigine.runtime.director.canvas.stage.prototype.close = function() {
    ...
    var self = this;
    return promise.animate(function (elapsed, duration) {
        if (1 == elapsed) state = self.$ctx.getImageData(0, 0, 1280, 720);
...
bigine.runtime.director.canvas.stage.prototype.type = function(title, avatar, words, theme) {
    ...
    return new promise(function (resolve) {
        var id = title,
            ...
            h = 720,
            state;
        if (avatar && theme.avatar) id += avatar.src;
        if (self.$text[id]) return resolve(self.$text[id]);
        self.$text[id] = new Image();
        bigine.util.on(self.$text[id], 'load', function () {
            resolve(self.$text[id]);
        });
        self.render();
        ...
        if (title && theme.name) self.$font(42, theme.name.color, theme.name.shadow, function () {
            this.fillText(title, theme.name.left, theme.name.top);
        });
        self.$text[id].src = self.canvas.toDataURL();
        self.$ctx.putImageData(state, 0, 0);
    }).then(function (image) {
        self.render();
        self.$ctx.drawImage(image, 0, 0);
        var auto = self.$runtime.auto,
            skip = false,
            offset = 0,
            pos = {
                x: theme.text.left,
            ...
            limit = theme.text.left + theme.text.width,
            t = function () {
                return self.type$char(chars[offset], pos, limit);
            },
            q = function () {
                if (auto) return;
                skip = true;
                for (offset++; offset < chars.length; offset++) pos = self.$font(
                    28,
        ...
        if ('#TIP' == title) {
            auto = false;
            limit = self.$font(28, '', 0, function () {
                return this.measureText(chars).width;
            });
        ...
        return q.then(function () {
            if (self.$runtime.auto) return promise.tick(chars.length * (delay - speed));
            return promise.click(self.canvas, [13]);
        });
    });
};
 ...
 * @return {{x, y, x0}}
 */
bigine.runtime.director.canvas.stage.prototype.type$char = function(chr, pos, limit) {
 ...
 */
bigine.runtime.director.canvas.stage.prototype.$font = function(size, color, shadow, callback, context) {
    size = size || 32;
    color = color || '#fff';
    shadow = shadow || 0;
    callback = bigine.util.isFunction(callback) ? callback : bigine.util.noop;
    context = context || this.$ctx;
    context.save();
    context.font = size + 'px "Hiragino Sans GB", "Microsoft YaHei", "WenQuanYi Micro Hei", Arial, sans-serif';
