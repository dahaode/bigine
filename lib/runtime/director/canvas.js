...
bigine.runtime.director.canvas.prototype.init = function(runtime) {
    ...
    var self = this,
        body = document.body,
        els = body.querySelectorAll('.bg-work'),
        autoplay = false,
    ...
    if (els.length) {
        ...
        el.className = 'bg-work';
    } else {
        autoplay = true;
        body.appendChild(el);
        el.className = 'bg-work fullscreen dynamic';
    }
...
/** @inheritDoc */
bigine.runtime.director.canvas.prototype.destroy = function() {
    this.stage = undefined;
    var box = document.body.querySelectorAll('.bg-work')[0];
    if (/\bdynamic\b/.test(box.className)) box.parentNode.removeChild(box);
    else box.innerHTML = '';
};

/** @inheritDoc */
bigine.runtime.director.canvas.prototype.click = function() {
    this.stage.canvas.click();
};
...
bigine.runtime.director.canvas.prototype.logo$show = function(url) {
    ...
    var self = this;
    return this.runtime.resource.get(url).then(function (image) {
        self.stage.setLayer(0, image);
    });
};
...
bigine.runtime.director.canvas.prototype.start$show = function(bg, btns) {
    ...
    var self = this;
    return $p.all([
        this.runtime.resource.get(bg),
        this.runtime.resource.get(btns[0].i),
        this.runtime.resource.get(btns[0].ih),
        this.runtime.resource.get(btns[1].i),
        this.runtime.resource.get(btns[1].ih)
    ]).then(function (images) {
        btns[0].i = images[1];
        btns[0].ih = images[2];
        btns[1].i = images[3];
        btns[1].ih = images[4];
        return self.stage.setLayer(0, images[0]).setStart(btns);
    });
};
...
bigine.runtime.director.canvas.prototype.map$show = function(base, points) {
    ...
    bigine.util.each(points, function (point) {
        q.push(self.runtime.resource.get(point.i));
    });
...
bigine.runtime.director.canvas.prototype.back$show = function(snap) {
    ...
    var self = this;
    return this.runtime.resource.get(snap).then(function (image) {
...
bigine.runtime.director.canvas.prototype.voiceover$show = function(theme, words) {
    ...
    var self = this;
    return this.runtime.resource.get(theme.back.image).then(function (image) {
        theme.back.image = image;
        return self.stage.type('#VOICEOVER', false, words, theme);
    });
};
...
bigine.runtime.director.canvas.prototype.monolog$show = function(theme, title, avatar, words) {
    ...
    var self = this;
    return $p.all([
        this.runtime.resource.get(theme.back.image),
        avatar ? this.runtime.resource.get(avatar.src) : false
    ]).then(function (images) {
...
bigine.runtime.director.canvas.prototype.chars$set$left = function(pose) {
    ...
    var self = this;
    return this.runtime.resource.get(pose).then(function (image) {
...
bigine.runtime.director.canvas.prototype.cg$show = function(image) {
    ...
    var self = this;
    return this.runtime.resource.get(image).then(function (image) {
...
bigine.runtime.director.canvas.prototype.choose$show = function(theme, options) {
    ...
    if (!options.length) return $p.resolve(false);
    var self = this;
    return $p.all([
        this.runtime.resource.get(options[0].i),
        this.runtime.resource.get(options[0].ih)
    ]).then(function (images) {
        var margin = 16,
...
bigine.runtime.director.canvas.prototype.bgm$play = function(id) {
    ...
    if (id) this.bgm.src = this.runtime.resource.get(id);
    return this.oops();
};
...
bigine.runtime.director.canvas.prototype.se$play = function(id) {
    ...
    if (id) this.se.src = this.runtime.resource.get(id);
    return this.oops();
};
...
bigine.runtime.director.canvas.prototype.end$show = function(base, cover) {
    ...
    var self = this;
    return $p.all([
        this.runtime.resource.get(base),
        this.runtime.resource.get(cover)
    ]).then(function (images) {
...
/** @inheritDoc */
bigine.runtime.director.canvas.prototype.rank$hide = function() {
