...
bigine.runtime.director.classic.prototype.init = function(runtime) {
    ...
    this.width = this.viewport.scrollWidth;
    ...
    runtime.episode.$on('ready', function () {
        ...
        if (autoplay) {
            runtime.logger.info('[runtime] AUTOPLAY');
            self.began = true;
        }
        if (self.began) {
            self.$play();
...
bigine.runtime.director.classic.prototype.$start$render = function() {
    ...
    this.viewport.$start.innerHTML = '<img src="' + theme.image + '">';
    this.viewport.$start.$new = this.viewport.$start.appendChild($.$element('img'));
    this.viewport.$start.$new.className = 'new';
    this.viewport.$start.$load = this.viewport.$start.appendChild($.$element('img'));
    this.viewport.$start.$load.className = 'load';
    ...
    var self = this,
        el = bigine.util.on(this.viewport.$map.appendChild($.$div('point')), {
            mouseenter: function() {
                ...
                self.viewport.$map.innerHTML = '<img src="' + $.$url(point.image.src, 'png') + '">';
            },
            mouseleave: function() {
                self.viewport.$map.innerHTML = '';
            },
            click: function() {
...
bigine.runtime.director.classic.prototype.voiceover = function(metas, context) {
    ...
    }).click(this.viewport.$voiceover, [13, 32]).then(function () {
...
bigine.runtime.director.classic.prototype.$chars$animate$middle = function(pose) {
    ...
    this.viewport.$chars.$middle.style.top = this.height * 0.235 + 'px';
    return promise.all([
...
bigine.runtime.director.classic.$climb = function(element, distance, frames) {
    return promise.tick(function (counter) {
        ...
        element.style.top = (distance / frames + element.offsetTop) + 'px';
    }, frames);
};
...
bigine.runtime.director.classic.$move = function(element, distance, frames) {
    return promise.tick(function (counter) {
        ...
        element.style.left = (distance / frames + element.offsetLeft) + 'px';
    }, frames);
};
...
bigine.runtime.director.classic.prototype.$chars$animate$right = function(pose) {
    ...
    this.viewport.$chars.$right.style.left = this.width * 0.7 + 'px';
    return promise.all([
...
bigine.runtime.director.classic.prototype.speak = function(metas, context) {
    ...
    }).click(this.viewport, [13, 32]).then(function () {
...
bigine.runtime.director.classic.prototype.$cg$show = function(image) {
    ...
    ]).click(this.viewport.$cg, [13, 32]).then(function () {
...
bigine.runtime.director.classic.$shrink = function(element, frames) {
    var c = 0,
        m = (1 + frames) * frames / 2,
        lf, tf, wf, hf, lt, tt,
        wt = element.scrollWidth,
        ht = element.scrollHeight;
    return promise.tick(function (counter) {
        if (1 == counter) {
            $.$show(element);
            wt = element.scrollWidth;
            ...
            lf = element.offsetLeft;
            tf = element.offsetTop;
            wf = element.scrollWidth;
            hf = element.scrollHeight;
        }
        c += counter;
        lt = (wf - wt) * counter / frames;
        tt = (hf - ht) * counter / frames;
        $.$style(element, {
            left: (lf + lt / 2) + 'px',
            top: (tf + tt / 2) + 'px',
            width: (wf - lt) + 'px',
...
bigine.runtime.director.classic.prototype.monolog = function(metas, context) {
    ...
    }).click(this.viewport.$monolog, [13, 32]).then(function () {
...
bigine.runtime.director.classic.prototype.tip = function(metas, context) {
    ...
    }).click(this.viewport.$tip, [13, 32]).pass(context);
};
...
bigine.runtime.director.classic.prototype.$chars$hide$left = function() {
    ...
    return promise.all([
        ...
        self.viewport.$chars.$left.style.left = 0.025 * self.width + 'px';
    });
};
...
bigine.runtime.director.classic.prototype.$chars$hide$middle = function() {
    ...
    return promise.all([
        ...
        self.viewport.$chars.$middle.style.top = 0.16 * self.height + 'px';
    });
};
...
bigine.runtime.director.classic.prototype.$chars$hide$right = function() {
    ...
    return promise.all([
        ...
        self.viewport.$chars.$right.style.left = 0.675 * self.width + 'px';
    });
};
...
bigine.runtime.director.classic.$choose$show$option = function(title, value) {
    ...
    if (theme) {
        if (theme.hover) {
            bigine.util.on(el.$_icon, {
                mouseenter: function() {
                    el.$_icon.innerHTML = '<img src="' + theme.hover + '">';
                },
                mouseleave: function() {
                    el.$_icon.innerHTML = '';
                }
...
bigine.runtime.director.classic.prototype.$choose$render = function() {
    ...
    this.viewport.$choose = this.viewport.appendChild($.$div('choose', {options: {}}));
...
bigine.runtime.director.classic.prototype.$se$play = function(audio) {
    this.$se$render();
    this.viewport.$se[0].src = $.$url(audio, 'mp3');
    return this.oops();
};
...
bigine.runtime.director.classic.prototype.$se$render = function() {
    ...
    this.viewport.$se = [];
    for (var ii = 0, jj; ii < 3; ii ++) {
        jj = $.$audio({
            autoplay: true,
            preload: true
        });
        jj.className = 'se';
        this.viewport.$se.push(jj);
        this.viewport.appendChild(jj);
    }
};
...
bigine.runtime.director.classic.prototype.$se$activate = function() {
    ...
    bigine.util.each(this.viewport.$se, function (audio) {
        audio.play();
    });
};
