...
bigine.runtime.director.classic.prototype.init = function(runtime) {
    ...
    var self = this,
        doc = window.document,
        els = doc.querySelectorAll('.bg-player'),
        size,
        el = $.$div('bg-player', {
            viewport: {}
        }),
...
bigine.runtime.director.classic.$div = function(className, children) {
    var el,
        cel = window.HTMLElement || window.Element;
    if (className instanceof cel) {
        el = className;
    } else {
        el = $.$element('div');
...
bigine.runtime.director.classic.$element = function(tagName) {
    var cel = window.HTMLElement || window.Element;
    if (tagName instanceof cel) {
        return tagName;
    }
...
bigine.runtime.director.classic.prototype.$speak$render = function() {
    ...
    var el = this.viewport.appendChild($.$div('words', {
        speak: {}
    }));
...
bigine.runtime.director.classic.prototype.$chars$animate$middle = function(pose) {
    ...
    $.$style(this.viewport.$chars.$middle, {
        backgroundImage: 'url(' + $.$url(pose, 'png') + ')',
        top: this.height * 0.235 + 'px'
    });
    return promise.all([
...
bigine.runtime.director.classic.prototype.$chars$animate$left = function(pose) {
    ...
    $.$style(this.viewport.$chars.$left, {
        backgroundImage: 'url(' + $.$url(pose, 'png') + ')',
        left: 0
    });
    var self = this;
...
bigine.runtime.director.classic.prototype.$chars$animate$right = function(pose) {
    ...
    $.$style(this.viewport.$chars.$right, {
        backgroundImage: 'url(' + $.$url(pose, 'png') + ')',
        left: this.width * 0.7 + 'px'
    });
    return promise.all([
...
bigine.runtime.director.classic.prototype.speak = function(metas, context) {
    ...
    }).click(this.viewport.$speak, [13, 32]).then(function () {
