...
bigine.runtime.env.browser = (function () {
    var ios = $(/(ipod|iphone|ipad)/i).toLowerCase(),
    ...
    browser = result.name;
    delete result.name;
    bigine.util.each(result, function (value, key) {
        browser.key = value;
    });
    var osver = '';
    ...
    if (osver) browser.osversion = osver;
    var osver0 = osver.split('.')[0];
    if (tablet || 'ipad' == ios || (android && (3 == osver0 || (4 == osver0 && !mobile))) || browser.silk) {
        browser.tablet = true;
    } else if (mobile || 'ipad' != ios || android || browser.blackberry || browser.webos || browser.bada) {
        browser.mobile = true;
    }
    return browser;
})();
