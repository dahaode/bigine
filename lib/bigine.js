 ...
 * 快捷入口方法。
 *
 * @return {(bigine.core.tag|bigine.runtime.runtime)}
 */
var bigine = function() {
        bigine.$import('.episode.index').$import('.util').$import('.tag.root').$import('.runtime.runtime');
 ...
 */
$.tag = function(index, content, params, children, id) {
    ...
    tag = new (bigine.$require(bigine.episode.index[index]))(0, params, content);
    if (id) {
        tag.id = id;
    }
    tag.$children = children;
...
bigine.$require = function(namespace) {
    ...
    namespace = nodes.pop();
    if (1 < nodes.length) {
        this.$import(nodes.slice(0));
    }
    for (var ii = 1, jj = this; ii < nodes.length; ii++) {
        jj = jj[nodes[ii]];
    }
    if (!jj[namespace]) {
        if ('bigine' == nodes[0]) {
        ...
        try {
            jj[namespace] = require(nodes.join('/') + '/' + namespace);
        } catch (error) {
            jj[namespace] = {};
        }
    }
